...
// POST /api/bookings → 예약 생성 (사용자/사업자 모두 가능)
router.post("/", createBooking);

// GET /api/bookings → 예약 목록 조회 (사용자/사업자 모두 가능)
router.get("/", getBookings);

// GET /api/bookings/:id → 예약 상세 조회 (사용자/사업자 모두 가능)
router.get("/:id", getBookingById);

// PATCH /api/bookings/:id/status → 예약 상태 변경 (사업자만 가능)
router.patch("/:id/status", requireBusiness, updateBookingStatus);

// PATCH /api/bookings/:id/payment → 결제 상태 변경 (사업자만 가능)
